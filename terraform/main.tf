...
locals {
  cluster_members = ["kafka-1", "kafka-2"]
}
...
resource "hcloud_server" "node" {
  ...
  user_data = <<-EOF
    #cloud-config
    runcmd:
    - curl -sL https://github.com/flynnt-io/flynnt-agent-install/releases/latest/download/flynnt.sh | API_KEY=${data.sops_file.secrets.data["flynnt_token"]} bash -s - install -c ${var.flynnt_cluster} -n ${each.key}
  EOF
}
